...
class List_Course_For_Client extends ArrayAdapter {
    ...
    public List_Course_For_Client(Activity context, List<vl_course> valueCourse) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        View ListViewItem = inflater.inflate(R.layout.layout_course_client,null, true);

        TextView txtcourse =(TextView)ListViewItem.findViewById(R.id.txtcoursenamefor);
        final TextView txtcoursecount = (TextView)ListViewItem.findViewById(R.id.txtcoursecountfor);

        vl_course vlCourse = valueCourse.get(position);
        txtcourse.setText(vlCourse.getCourseName().toString());

        refCourse= FirebaseDatabase.getInstance().getReference("Chapters").child(CourseTypeIDForChapterFor).child(vlCourse.getCourseId());
